...
class MAXIDSolver:
    ...
    def _create_compressed_graph(self, compression_map: Dict[int, Tuple[int, int]]) -> nx.Graph:
        """Create a compressed graph from a compression map."""
        G_prime = self.tree.to_nx()
        for v, (u, w) in compression_map.items():
...
class ParallelMAXIDSolver(MAXIDSolver):
    ...
    def _process_machine_compression(self, machine_id: int, partition: Set[int], 
        ...
        try:
            compression_map = {}
            active_nodes = [v for v in partition if self.nodes[v]['state'] == "active"]
            ...
            for node_id in active_nodes:
                ...
                if isinstance(result, tuple) and len(result) == 3:
                    v, state, target = result
                    if state == "happy" and target is not None:
                        # Only compress if both nodes are in this machine's partition
                        # or if the target is a boundary node
                        if target in partition or target in boundary_nodes:
                            compression_map[v] = target
                            self.nodes[target]['compressed_nodes'].add(v)
                            self.nodes[v]['compressed_into'] = target
                            self.nodes[v]['state'] = "happy"
                else:
                    logger.warning(f"Unexpected result format for node {node_id}: {result}")
                ...
                try:
                    ...
                    for machine_id in range(self.num_machines):
                        ...
                        future = self.client.submit(
                            self._process_machine_compression,
                            machine_id,
                            partition,
                            boundary_nodes,
                            machine_graph,
                            pure=False  # Allow non-pure functions
                        )
    ...
    def _visualize_progress(self, phase: int, graph: nx.Graph):
        ...
        try:
            ...
            # Color nodes based on their max_id
            node_colors = [self.nodes[n]['max_id'] for n in graph.nodes()]
            cmap = plt.cm.viridis
    ...
    try:
        ...
        try:
            if client is not None:
                try:
                    client.wait_for_workers(num_machines, timeout=5)
